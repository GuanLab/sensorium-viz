...
import numpy as np
import torch
from dataloaders.sensorium_test import SensoriumDataset
...
def main(args, split):
    ...
    if args.add_behavior:
        logs["addition_info"].append("behavior")
        num_channel += 3
    if args.add_pupil:
        logs["addition_info"].append("pupil_center")
        num_channel += 2
    ...
    im_dataset = SensoriumDataset(args.mouse_ids, embed_grids=32, 
                                  add_behavior_as_channel=args.add_behavior,
                                  add_pupil_center_as_channel=args.add_pupil,
                                  norm_coord=args.norm_coord, 
                                  custom_test_split=args.test_file)
    im_loader = DataLoader(im_dataset, batch_size=batch_size, shuffle=True, drop_last=False)
    ...
    for count, (gt_image, cond) in enumerate(im_loader):
        if limit is not None and (count+1) * batch_size > limit:
            break
...
if __name__ == "__main__":
    ...
    parser.add_argument("--add-behavior", action="store_true")
    parser.add_argument("--add-pupil", action="store_true")
